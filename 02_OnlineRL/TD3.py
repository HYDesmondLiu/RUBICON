...
class TD3(object):
	...
	def train(self, replay_buffer, env, batch_size=256):
		self.total_it += 1
		...
		if self.total_it % self.policy_freq == 0:
			...
			Q_buffer_mean = Q_buffer.mean().item()
			Q_rbc_mean = Q_rbc.mean().item()

			if Q_buffer_mean >= Q_rbc_mean:
